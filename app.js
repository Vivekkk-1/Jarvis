...
function speak(text) {
    ...
    // Adjust the rate, volume, and pitch
    text_speak.rate = 1;
    text_speak.volume = 1;
    text_speak.pitch = 1;
    ...
    const selectedVoice = voices.find(voice => voice.name === "Google UK English Male"); // Change this to the voice you want
    if (selectedVoice) {
...
function takeCommand(message) {
    if (message.includes('hey') || message.includes('hello')) {
        ...
        window.open('https://www.rapidtables.com/tools/notepad.html');
...
// Sending messages via input field and executing commands
...
textInput.addEventListener('keydown', (e) => {
    if (e.key === "Enter") {
        sendBtn.click(); // Trigger send on Enter
    }
});
